...
class CursorGUI {
    constructor() {
        ...
        this.lastClickX = 0;
        this.lastClickY = 0;
        this.doubleClickThreshold = 500; // ms para detectar doble click (estándar es ~500ms)
        this.doubleClickDistanceThreshold = 30; // Más tolerancia para el doble click
        this.clickCount = 0;
        ...
        this.pressStartX = 0;
        this.pressStartY = 0;
        this.movementThreshold = 30; // Más tolerancia para PC/Touch (20 -> 30)
        this.pressStartTime = 0;
        this.longPressThreshold = 600; // Más rápido para mejor sensación (800 -> 600)
    ...
    startLongPress(x, y) {
        ...
        this.pressStartTime = currentTime; // Guardar tiempo de inicio para long press
        this.isPressing = true;
        this.pressStartX = x; // Guardar posición EXACTA del inicio
        this.pressStartY = y;
        this.centerX = x;
        this.centerY = y;
    }
    ...
    updatePosition(x, y) {
        if (this.isPressing && !this.isVisible) {
            ...
            if (!this.checkIfStill()) {
                ...
                // Si ha pasado suficiente tiempo quieto, mostrar
                const elapsed = Date.now() - this.pressStartTime;
                if (elapsed >= this.longPressThreshold) {
                    this.show(this.pressStartX, this.pressStartY);
                }
            }
        }
    }
    ...
    /**
     * Dibujar el progreso del long press (arco circular)
     */
    drawLongPressProgress(buffer) {
        ...
        // Sombra / Fondo sutil del arco
        buffer.stroke(0, 0, 0, 100);
        buffer.strokeWeight(6);
        buffer.ellipse(this.pressStartX, this.pressStartY, 50, 50);
        ...
        buffer.arc(this.pressStartX, this.pressStartY, 50, 50, startAngle, endAngle);

        // Círculo central latente
        const pulse = Math.sin(frameCount * 0.1) * 5;
        buffer.fill(138, 79, 191, 150 + pulse * 10);
        buffer.noStroke();
        buffer.ellipse(this.pressStartX, this.pressStartY, 10 + pulse, 10 + pulse);
    ...
    display(buffer) {
        if (!this.isVisible) {
            if (this.isPressing) {
                this.drawLongPressProgress(buffer);
            }
            return;
        }
...
// Crear instancia global INMEDIATAMENTE para evitar 404/Null
if (typeof window !== 'undefined') {
    window.cursorGUI = new CursorGUI();
    console.log('🚀 CursorGUI: Instancia global creada (Ready for action)');
}
